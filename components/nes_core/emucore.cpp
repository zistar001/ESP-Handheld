...
int rom_allocsram(rominfo_t **rominfo) {
  ...
  if (NULL == (*rominfo)->sram) (*rominfo)->sram = (uint8_t *)heap_caps_malloc(SRAM_BANK_LENGTH * (*rominfo)->sram_banks, MALLOC_CAP_8BIT);
  if (NULL == (*rominfo)->sram) {
    ///gui_sendmsg(GUI_RED, "Could not allocate space for battery RAM");
    return -1;
  }
...
mmc_t *mmc_temp;  /* must be declared before mmc_destroy uses it */
...
void mmc_destroy(mmc_t **nes_mmc)
{
    if (*nes_mmc)
        free(*nes_mmc);
    *nes_mmc = NULL;
    mmc_temp = NULL;  /* prevent dangling ref — next mmc_create allocates fresh */
}
mapintf_t **map_ptr;

mmc_t *mmc_create(rominfo_t *rominfo)
{
    for (map_ptr = (mapintf_t **)mappers; *map_ptr != NULL; map_ptr++)
    {
        if ((*map_ptr)->number == rominfo->mapper_number)
            break;
    }
    if (NULL == *map_ptr) {
...
nes_t *nes_create(void)
{
    int i;
    if (NULL == machine) {
        machine = (nes_t *)heap_caps_malloc(sizeof(nes_t), MALLOC_CAP_8BIT);
        if (NULL == machine)
            machine = (nes_t *)heap_caps_malloc(sizeof(nes_t), MALLOC_CAP_8BIT);
    }
    if (NULL == machine)
    ...
    // cpu
    if (NULL == machine->cpu) {
        machine->cpu = (nes6502_context *)heap_caps_malloc(sizeof(nes6502_context), MALLOC_CAP_8BIT);
        if (NULL == machine->cpu)
            machine->cpu = (nes6502_context *)heap_caps_malloc(sizeof(nes6502_context), MALLOC_CAP_8BIT);
    }
    if (NULL == machine->cpu)
    ...
    if (NULL == machine->cpu->mem_page[0])
        machine->cpu->mem_page[0] = (uint8_t *)heap_caps_malloc(NES_RAMSIZE, MALLOC_CAP_8BIT);
    if (NULL == machine->cpu->mem_page[0])
        goto _fail;
...
int NES_state_load(char* filename) {
    (void)filename;
    /* Save state loading requires SD_MMC (Arduino API), stub for ESP-IDF */
    return -1;
}
