...
void rg_system_clear_panic(void)
{
...
__attribute__((weak)) void rg_system_restart(void)
{
...
__attribute__((weak)) void rg_system_exit(void)
{
    RG_LOGW("Exiting application!");
    /* Clear boot config and restart same partition → goes to launcher */
    rg_settings_set_string(NS_BOOT, SETTING_BOOT_NAME, NULL);
    rg_settings_set_string(NS_BOOT, SETTING_BOOT_ARGS, NULL);
    rg_settings_set_number(NS_BOOT, SETTING_BOOT_FLAGS, 0);
    rg_settings_commit();
    esp_restart();
}
