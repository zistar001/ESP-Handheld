...
// Non-static: combo key injection in rg_system_tick_hook (main.c) needs access
uint32_t gamepad_state = -1; // _Atomic
...
bool rg_input_read_gamepad_raw(uint32_t *out)
{
    ...
    /* ESP-BSP Handheld combo keys (7 physical buttons → 8+ virtual) */
#if defined(RG_TARGET_ESP_BSP_HANDHELD)
    static int64_t start_press_ms = 0;
    static bool start_was_pressed = false;
    int64_t now = esp_timer_get_time() / 1000;

    /* START + A = SELECT */
    if ((state & RG_KEY_START) && (state & RG_KEY_A)) {
        state |= RG_KEY_SELECT;
        state &= ~(RG_KEY_A | RG_KEY_START); /* consume both so they don't double-fire */
    }

    /* Track START press timing for long-press → MENU */
    if (state & RG_KEY_START) {
        if (!start_was_pressed) {
            start_was_pressed = true;
            start_press_ms = now;
        }
        /* START + B held ≥150ms → Return to ESP_BSP (factory) */
        if ((state & RG_KEY_B) && (now - start_press_ms) >= 150) {
        ...
        /* START held alone ≥1000ms → MENU */
        if (!(state & RG_KEY_B) && (now - start_press_ms) >= 1000) {
            state |= RG_KEY_MENU;
        }
    } else {
